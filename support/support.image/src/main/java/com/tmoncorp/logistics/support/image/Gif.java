...
import in.java.support.image.Gif.GifFrame.DisposalMethod;
...
//	}
	...
	Gif(ImageReader reader, Format format) throws IOException
	{
		super(reader, format);
...
//    			System.out.println(i+" = "+currentImage.getWidth()+ " / "+currentImage.getHeight()+ " "+point.x+" : "+point.y);
    			...
    			try
				{
					frame.saveAs(new File("C:/Temp/test"+i+".jpg"), Format.JPEG);
				} catch (IOException e)
				{
...
//		public static final int TRANSPARENCY_COLOR_NONE = -1;
	...
	{
		private Format format = Format.GIF;
