...
import in.java.support.api.spring.annotation.JsonView;
import in.java.support.api.spring.annotation.JsonViews;
 ...
 */
class JsonViewHandler implements HandlerMethodReturnValueHandler {

    //로그
    static Logger log = LoggerFactory.getLogger(JsonViewHandler.class);
    ...
    //controllerAdvices 에 선언된 JsonView 리스트
    private final Map<Class<?>, JsonView> adviceJsonFilters = new HashMap<Class<?>, JsonView>();

    private LoadingCache<Class<?>, Map<Class<?>, JsonView>> CONTROLLER_CACHE = CacheBuilder.newBuilder().weakKeys()
            .refreshAfterWrite(5, TimeUnit.MINUTES)
            .build(new CacheLoader<Class<?>, Map<Class<?>, JsonView>>() {
                @Override
                public Map<Class<?>, JsonView> load(Class<?> containingClass) {
                    return controllerAnnotations(containingClass);
                }
            });

    private LoadingCache<MethodParameter, Map<Class<?>, JsonView>> METHOD_CACHE = CacheBuilder.newBuilder().weakKeys()
            .refreshAfterWrite(10, TimeUnit.MINUTES)
            .build(new CacheLoader<MethodParameter, Map<Class<?>, JsonView>>() {
                @Override
                public Map<Class<?>, JsonView> load(MethodParameter methodInfo) {
    ...
    static class MethodMeta {
        ...
        public MethodMeta(final MethodParameter methodInfo,
            ...
            //파라미터 필터 기능 추가
            String paramFields = webRequest.getParameter("_exclude_");
            if(paramFields != null ){
//                Set<String> parameterFilters = new HashSet<String>();
     ...
     */
    public JsonViewHandler(final HandlerMethodReturnValueHandler delegate, final ApplicationContext applicationContext) {
    ...
    public void handleReturnValue(final Object returnValue, final MethodParameter methodInfo,
        ...
        final Map<Class<?>, JsonView> jsonViewMap = METHOD_CACHE.get(methodInfo);
//        final Map<Class<?>, JsonView> jsonViewMap = methodAnnotations(methodInfo);
    ...
    private Object processValue(final DataBucket bucket, final MethodMeta meta, final Map<Class<?>, JsonView> jsonViewMap) {
        ...
        //class 타입 인 경우
        //final DataBucket bucket, final MethodMeta controllerInfo, final Map<Class<?>, JsonView> jsonViewMap
        return filterData(bucket, meta, jsonViewMap);
    }

    /**
     * Filter data. JsonView 어노테이션 필터 처리
     */
    @SuppressWarnings({
            "unchecked", "rawtypes"
    })
    private Object filterData(final DataBucket bucket, final MethodMeta meta, final Map<Class<?>, JsonView> jsonViewMap) {
        ...
        if (jsonViewMap.size() > 0) {
            ...
            String[] fields = null;
            JsonView jsonView = jsonViewMap.get(bucket.type);
            if (jsonView != null) {
                fields = jsonView.fields();
            } else {
                //반환 타입과 핕터 타입이 일치하지 않는 경우
                //반환 값의 클래스 타입가 필터 타입 매칭 시도
                Class<?> returnValueClass = bucket.value.getClass();
                jsonView = jsonViewMap.get(returnValueClass);
                if (jsonView != null) {
                    fields = jsonView.fields();
                ...
                for(Template template : _templates) {
                    ...
                    switch (template.type) {
                        ...
                        case Method:
                            ...
                            break;
                        case MaV:
                            result = ((Function) template.value).apply(new JsonView.Meta());
                            break;
                        case Data:
...
//            Pattern methodPattern = Pattern.compile("\\p{Alpha}+\\w?\\(\\s?\\)$");
    ...
    private void adviceAnnotations() {
        ...
        final Map<Class<?>, JsonView> objectMapper = new HashMap<Class<?>, JsonView>();
        // controllerAdvices 필터 맵핑
        for (Object advice : controllerAdvices.values()) {
            Map<Class<?>, JsonView> adviceJsonFilter = controllerAnnotations(advice.getClass());
    ...
    /**
     * controller class에 선언된 JsonView
     *
     * @param controllerClass controller class
     * @return
     */
    private Map<Class<?>, JsonView> controllerAnnotations(Class<?> controllerClass) {
        final Map<Class<?>, JsonView> objectMapper = new HashMap<Class<?>, JsonView>();

        final JsonViews jsonViews = controllerClass.getAnnotation(JsonViews.class);
        if (jsonViews != null) {
            for (final JsonView filter : jsonViews.value()) {
                objectMapper.put(filter.value(), filter);
            }
        } else {
            final JsonView jsonView = controllerClass.getAnnotation(JsonView.class);
            if (jsonView != null) {
                objectMapper.put(jsonView.value(), jsonView);
            }
        }
        return objectMapper;
    }
     ...
     * @return
     */
    private Map<Class<?>, JsonView> methodAnnotations(final MethodParameter methodInfo) {
        final Map<Class<?>, JsonView> jsonViewMap = new HashMap<Class<?>, JsonView>();
        if (this.adviceJsonFilters.size() > 0) {
            jsonViewMap.putAll(this.adviceJsonFilters);
        }
        // Controller 클래스 필터 캐쉬
        try {
            Map<Class<?>, JsonView> classFilters = CONTROLLER_CACHE.get(methodInfo.getContainingClass());
            if (classFilters != null && classFilters.size() > 0)
                jsonViewMap.putAll(classFilters);
        } catch (ExecutionException e) {
            log.error("Controller Class JsonView cache error", e);
            /*Map<Class<?>, JsonView> classFilters = controllerAnnotations(returnType.getContainingClass());
            if (classFilters != null && classFilters.size() > 0)
                jsonViewMap.putAll(classFilters);*/
        }

        final JsonViews jsonViews = methodInfo.getMethodAnnotation(JsonViews.class);
        if (jsonViews != null) {
            for (final JsonView filter : jsonViews.value()) {
                jsonViewMap.put(filter.value(), filter);
            }
        } else {
            final JsonView jsonView = methodInfo.getMethodAnnotation(JsonView.class);
            if (jsonView != null) {
                jsonViewMap.put(jsonView.value(), jsonView);
            }
        }
        return jsonViewMap;
    }
